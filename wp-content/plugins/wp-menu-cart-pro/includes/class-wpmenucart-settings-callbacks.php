...
class WPO_Menu_Cart_Pro_Settings_Callbacks extends WPO_Settings_Callbacks_2 {
	...
	public function menus_select( $args ) {
		...
		$menu_count = apply_filters( 'wpmenucart_menu_count', count( $current ) );
		if ($menu_count < 3) {
			$menu_count = 3;
		}
		...
		echo '<button class="button" id="add_wpmenucart_menu" style="margin-top:5px">'.__('Add menu', 'wp-menu-cart').'</button>';
	}
	...
	public function icons_radio_element_callback( $args ) {
		...
		$html .= '<p class="description"><i>'. __('<strong>Please note:</strong> you need to open your website in a new tab/browser window after updating the cart icon for the change to be visible!','wp-menu-cart').'</p>';
